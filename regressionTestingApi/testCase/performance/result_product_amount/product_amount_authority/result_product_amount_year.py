...
class result_product_amount_year:
    def __init__(self):
    ...
    def check_data_user(self,i =0,_users=['设计部-用户1', '销售部-用户5', '同部门下属-用户7']):
        ...
        Actual_result_all = _result.gole_completion(from_date=self.date_list[i][0], to_date=self.date_list[i][1], type='product_money', dimension='year',
                     data=self.date[i][0])
        # # # Expected_result输出两个值 中间值，表格  合同 商机 金额
        Expected_result_all =_result.product_amount_get_completion_year(_months=self.month[i], _users=_users, _month_view=self._month_view[i])
        # # # # 比较图表的统计值是否相同
        ...
        for i in range(s):
            ...
            if test_result == True:
                f = codecs.open('amount_y.txt', 'a', 'utf-8')
    ...
    def payments_amount_baobiao(self):
        _result_count = result_product_amount_year()
...
if __name__ == '__main__':
    _result_count =result_product_amount_year()
    if os.path.exists('amount_y.txt') ==True:
        os.remove('amount_y.txt')
