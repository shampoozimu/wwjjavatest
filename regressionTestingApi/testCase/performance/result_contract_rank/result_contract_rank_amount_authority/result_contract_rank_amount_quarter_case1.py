...
class result_contract_rank_amount_quarter:
    def __init__(self):
    ...
    def check_data_user(self,i,s1,_users=['设计部-用户1','销售部-用户5','同部门下属-用户7']):
        ...
        Actual_result_all = _result.gole_completion(from_date=self.date_list[i][0], to_date=self.date_list[i][1], type='contract_rank', dimension='quarter',
                     data='other')
        # # Expected_result输出两个值 中间值，表格
        Expected_result_all =_result.payments_amount_get_completion_quarter(_months=self.month[i], _users=_users, _month_view=self._month_view[i],s=s1)
        # # # 比较图表的统计值是否相同
        ...
        for i in range(s):
            ...
            if test_result == True:
                f = codecs.open('contract_rank_amount_q.txt', 'a', 'utf-8')
    ...
    def payments_amount_baobiao(self,s1):
        _result_count = result_contract_rank_amount_quarter()
...
if __name__ == '__main__':
    _result_count =result_contract_rank_amount_quarter()
    if os.path.exists('contract_rank_amount_q.txt') ==True:
        os.remove('contract_rank_amount_q.txt')
    #工作台结果对比
