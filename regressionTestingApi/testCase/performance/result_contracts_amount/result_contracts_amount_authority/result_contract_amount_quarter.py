...
class result_contract_amount_quarter:
    def __init__(self):
    ...
    def check_data_user(self,i =0,_users=['设计部-用户1','销售部-用户5','同部门下属-用户7']):
        ...
        Actual_result_all = _result.gole_completion(from_date=self.date_list[i][0], to_date=self.date_list[i][1], type='win_money', dimension='quarter',
                     data='other')
        # # Expected_result输出两个值 中间值，表格
        Expected_result_all =_result.contract_amount_get_completion_quarter(_months=self.month[i], _users=_users, _month_view=self._month_view[i])
        # # # 比较图表的统计值是否相同
    ...
    def payments_amount_baobiao(self):
        _result_count = result_contract_amount_quarter()
