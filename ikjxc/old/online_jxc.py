...
class OrderPurchase:
    ...
    #    #s.headers.update({'Cookie': self.cookie})
    #    s.headers.update({'Cache-Control': 'max-age=0, private, must-revalidate'})
    #    s.headers.update({'Cache-Control': 'max-age=0, private, must-revalidate'})
    #    #'Accept': '*/*;q=0.5text/javascript, application/javascript, application/ecmascript, application/x-ecmascript
    ...
    #    s.headers.update({'Server':'Tengine'})
    #    s.headers.update({'X-CSRF-Token': self.csrf})
    #    s.headers.update({'Transfer-Encoding': 'chunked'})
    #    s.headers.update({'Vary': 'Accept-Encoding'})
    #    s.headers.update({'Vary': 'Accept-Encoding'})
    #    s.headers.update({'X-Content-Type-Options': 'nosniff'})
    #    s.headers.update({'X-Request-Id': '319d02bd-1624-4c6c-b8b0-b5cc7ea9b6d2'})
    #    s.headers.update({'X-Runtime': '1.962097'})
    ...
    def post_response_json(self, url, body, content):
        ...
        for tmp_str in tmp_list:
            if 'uid=' in tmp_str:
                authorization += tmp_str
        s.headers.update({'Authorization': 'Token token=1813e4f38ada3802fc740a6431d7b8, uid=17d8c3460df2570a3dcf'})
        #
        #token=; uid=
        print s.headers
        response = s.post(url, data=body)
    ...
    def add_product(self):
        ...
        'product[name]':'毛巾',
        'product[product_category_id]':'81210',
        'product[spec]':'张',
        'product[note]':'wl-test-auto-%s'% self.get_random_int(9999),
        'product[unit_id]':'55648',
        'product[unit_name]':'瓶',
        'product[default_unit_id]':'55648',
        ...
        'product[life_period]':'',
        'product[product_units_attributes][0][unit_id]':'55648',
        'product[product_units_attributes][0][unit_name]':'瓶',
        'product[product_units_attributes][0][price_policy_attributes][sale_price]':'12',
        'product[product_units_attributes][0][price_policy_attributes][purchase_price]':'15',
        ...
        'product[product_images_attributes][0][fileID]':'o_1bido29kpeac8pb1mkq1svhhbgh',
        'product[product_images_attributes][0][filename]':'crm.jpg',
        'product[product_images_attributes][0][url]':'https://obbsip92b.qnssl.com/Fubprm9jg6lmNUfh1fDCukoE2e8m?&e=1497261244&token=w2yRBQw0CPY-fsy6hxBjASopjHCTtT7RKFRtQeJz:BIW_Kmj_J7saZS0kReYEyhSG9s4='
    ...
    def get_product_price_warehouse_info(self, product_id):
        ...
        if not success:
            return {}
        #print self.response.json()['inventories'][0]['inventory_in_warehouses']
        #for item in self.response.json()['inventories'][0]['inventory_in_warehouses'] :
        #    if item['quantity'] != '':
        #        print item['warehouse_id'] , item['quantity']
        #print self.response.json()
        #print self.response.json()['counts']['all_warehouses_total_quantity']
        #print self.response.json()['counts']['total_cost_amount']
        #print self.response.json()['inventories'][0]['cost_price']
        return self.response.json()
...
if __name__ == '__main__':
    ...
    cookie = '_invoicing_session=%3D%3D--5aaf3835a8ea62c84d1065e6c8315d0e2dc0d713; path=/; HttpOnly'
